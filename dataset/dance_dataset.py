...
class AISTPPDataset(Dataset):
    def __init__(
        ...
        pose_input, cond_input = self.process_dataset(data["pos"], data["q"], data['features'])
        self.data = {
            "pose": pose_input,
    ...
    def process_dataset(self, root_pos, local_q, cond):
        ...
        if self.train:
            print(global_pose_vec_input.shape, cond.shape)
            self.normalizer = Normalizer(global_pose_vec_input)
            self.cond_normalizer = Normalizer(cond)
        else:
            assert self.normalizer is not None
            assert self.cond_normalizer is not None
        ...
        print(f"{data_name} Dataset Cond Features Dim: {cond.shape}")

        return global_pose_vec_input, cond
